...
mkForAllCo tv kind_co co
  | GRefl r ty Nothing <- co
  , GRefl _ _ Nothing <- kind_co
  = GRefl r (mkInvForAllTy tv ty) Nothing
  | otherwise
  = ForAllCo tv kind_co co

-- | Make nested ForAllCos
mkForAllCos :: [(TyVar, Coercion)] -> Coercion -> Coercion
mkForAllCos bndrs (GRefl r ty Nothing)
  = let (refls_rev'd, non_refls_rev'd) = span (isReflCo . snd) (reverse bndrs) in
    foldl (flip $ uncurry ForAllCo)
          (GRefl r (mkInvForAllTys (reverse (map fst refls_rev'd)) ty) Nothing)
          non_refls_rev'd
...
mkHomoForAllCos tvs (GRefl r ty Nothing)
  = GRefl r (mkInvForAllTys tvs ty) Nothing
...
mkKindCo co
  ...
  , let tk1 = typeKind ty1
        tk2 = typeKind ty2
  , tk1 `eqType` tk2
  -- the kinds of @tk1@ and @tk2@ are the same, thus @eqTypeK@
...
mkProofIrrelCo r (GRefl _ _ Nothing) g  _  = mkReflCo r (mkCoercionTy g)
...
promoteCoercion co = case co of
    ...
    _ | ki1 `eqType` ki2
    -- @ki1@ and @ki2@ have the same kind, thus @eqTypeK@
...
castCoercionKind g r t1 t2 h1 h2
  ...
  | isReflCo h1                 = g `mkTransCo` mkGReflRightCo r t2 h2
  | isReflCo h2                 = mkGReflLeftCo r t1 h1 `mkTransCo` g
