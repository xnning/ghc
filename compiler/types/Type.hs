...
expandTypeSynonyms ty
  ...
  where
    ...
    go_co subst (GRefl r ty co)
      = mkGReflCo r (go subst ty) (go_co subst co)
...
mapCoercion mapper@(TyCoMapper { tcm_smart = smart, tcm_covar = covar
  ...
  where
    ...
    go (GRefl r ty co) = mkgreflco r <$> mapType mapper env ty <*> (go co)
...
tyConsOfType ty
  ...
  where
     ...
     go_co (GRefl _ ty co)         = go ty `unionUniqSets` go_co co
